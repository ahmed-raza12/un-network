...
function CreateStaff() {
  ...
  const staffReducer = useSelector((state) => state.staff.staff);
  const staff = location.state;
  console.log(staffReducer.length, 'dealerId');
  const validate = () => {
    let tempErrors = {};
    ...
    <Box mt={80} sx={{ minHeight: "100vh", backgroundColor: '#f4f6fd', padding: 4, borderRadius: 2, maxWidth: "auto", margin: 'auto' }}>
        {
          staffReducer.length < 10 ? (
            ...
            <Typography variant="h5" sx={{ mb: 3, color: colors.primary }}>
              You have reached the maximum limit of 10 staff members.
            </Typography>
          )
        }
      <Snackbar
